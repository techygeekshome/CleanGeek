# Chocolatey package for CleanGeek

The package source for `cleangeek` on the Chocolatey community repository lives here so that
`packageSourceUrl` in the nuspec points at something real.

## What the package does

It downloads `CleanGeekSetup.exe` from the GitHub release for the matching tag and verifies it against a
...
2. Take the SHA-256 for `CleanGeekSetup.exe` from the release's own `SHA256SUMS.txt`.
...
5. Install locally from the built nupkg and check the shim works:
   `choco install cleangeek -s . -y`
6. `choco push cleangeek.<version>.nupkg --source https://push.chocolatey.org/`
