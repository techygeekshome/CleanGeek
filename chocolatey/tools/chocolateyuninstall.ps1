...
$key = Get-UninstallRegistryKey -SoftwareName 'CleanGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'CleanGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for CleanGeek. Remove it manually rather than guessing:"
